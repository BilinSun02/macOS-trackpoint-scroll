...
while IFS=' ' read -r oid path; do
    ...
    if git cat-file blob "$oid" | LC_ALL=C grep -aEq "$SECRET_RE"; then
...
if git log --all --format='%H%n%B' | LC_ALL=C grep -aEq "$SECRET_RE"; then
...
if [ -d core ] && git -C core rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    if git -C core ls-files | LC_ALL=C grep -Eq '(^|/)(\.env($|\.)|id_(rsa|dsa|ecdsa|ed25519)$|credentials($|\.)|.*\.(p12|pfx|key|pem|mobileprovision)$)'; then
        echo 'suspicious credential-bearing path in core submodule HEAD' >> "$TMP_FINDINGS"
    fi
    if git -C core grep -I -q -E "$SECRET_RE" HEAD -- .; then
